...
rIndex = (1:nc);
nIndex = [75 + 24*100, 76 + 24*100, 75 + 25*100, 75 + 25*100];
%nIndex = [(nx/2+nx/4) + (ny/2)*nx, (nx/2+nx/4)+1+(ny/2)*nx,...
%          (nx/2+nx/4) + (ny/2-1)*nx, (nx/2+nx/4) +1+ (ny/2-1)*nx];
...
switch initcase
  case 1
      ...
      edges = [1:nx, (1:ny-2)*nx + 1,(1:ny-2)*nx + nx/2, ...
              (1:ny-2)*nx + nx/2+1, (2:ny-1)*nx, (1:nx) + nx*(ny-1)];
      
      hole = [((ny/2-1):(ny/2+2))*nx, ((ny/2-1):(ny/2+2))*(nx)+1] + nx/2;
...
for istate = 1 : numel(states)
    ...
    if istate == 1
         ...
         imwrite(imind2,cm2,'ScaledCrossTalkR.gif','gif','DelayTime',framerate, 'Loopcount',inf);
